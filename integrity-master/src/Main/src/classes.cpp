...
Parameters::Parameters()
{
	...
	min_cluster_size= 15; //min # of points of each cluster. If less than this -> remove cluster
	min_plane_size= 100; // min # of points of each plane. If less than this -> do not extract plane
	min_density= 250; // min density of points of each cluster. If less dense than this -> remove cluster
	min_cluster_distance= 0.8; // when clustering, this is the minimum distance in between two clusters. If the distance is less than this, they won't be divide into two clusters
	xlim= 5.51; // points with x coordinate farther than this are removed
	ylim= 5.51; // points with y coordinate farther than this are removed
	zlim= -0.1; // points lower than 0.5m from the LIDAR height are removed (remove the ground points)
	...
	min_cylinder_size= 15; // extract cylinders with at least this # of points
	min_association_distance= 2.5; // a cylinder is associated to a landmark in the map if is closer than this distance
	rlim= 5.51; // points farther than this distance from the LIDAR are removed
}
